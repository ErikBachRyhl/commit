...
**Commit** = **Reinforce** + **Enforce** + French wordplay (commitr = to strengthen)
...
- **Commitr** (French) = to strengthen, fortify, reinforce
...
# NEW
from commit.config import AppConfig
from commit.processor import process_repository
```

### Documentation
All documentation files have been updated:
- References to "AnkiTex" → "Commit"
...
```bash
cd /path/to/commit  # (formerly AnkiChat)
...
Just replace `anki-tex` with `commit` in your mental model!
...
|--------|-----|-----|
| Name | AnkiTex | Commit |
| Package | `anki_tex` | `commit` |
| Command | `anki-tex` | `commit` |
| Config | `anki-tex.yml` | `commit.yml` |
| State | `~/.anki_tex_state.json` | (unchanged) |
| GitHub | - | github.com/ErikBachRyhl/commit |

**Welcome to Commit - a tool to commit your learning! 🚀**
